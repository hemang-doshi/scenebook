...
export default function EditorHomePage() {
    ...
    fetch("/api/workspace")
      ...
      .then((data) => {
        const cards = (data.cards ?? []).map(
          (c: { id: string; title: string; status: string; format: string }) => ({
            id: c.id,
            title: c.title,
            status: c.status,
            format: c.format,
  ...
  return (
    <div className="relative flex min-h-screen bg-[#08080a] text-foreground overflow-hidden flex-col">
      ...
      {/* Main content wrapper with smooth entry animation */}
      <motion.div
          ...
          <section className="space-y-4">
            ...
            {isLoading ? (
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="h-36 animate-pulse rounded-xl bg-black/35 border border-border/40" />
                ))}
              </div>
            ) : projects.length === 0 ? (
                ...
                {projects.map((p) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="text-xs font-mono uppercase tracking-wider text-foreground group-hover:text-accent transition duration-200">{p.title}</h3>
                        <p className="mt-1.5 text-[9px] font-mono uppercase tracking-widest text-muted-foreground">{p.format || "Shorts"} · {p.status || "Draft"}</p>
                      </div>
