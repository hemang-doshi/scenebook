...
async function getSupabaseSnapshot(): Promise<WorkspaceSnapshot> {
  ...
  return {
    ...
    inboxItems: inboxRows.map((item) => ({
      ...
      title: item.title,
      notes: item.notes,
      sourceType: item.source_type as "text" | "link" | "reference" | "voice",
...
export async function getProjectWorkspace(cardId: string): Promise<ProjectWorkspace | null> {
  ...
  const mappedGenerations = ((generations ?? []) as GenerationRecordRow[])
    ...
    .map<GenerationRecord>((generation) => ({
      ...
      modality: generation.modality as "text" | "image" | "audio" | "video",
      prompt: generation.prompt,
      status: generation.status as "queued" | "completed" | "failed",
